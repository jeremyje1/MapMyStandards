#!/usr/bin/env node
// Optional OG image generator. The heavy native dependency (canvas) was removed from
// production dependencies to avoid build failures on Vercel.
// To use locally:
//   npm install --save-dev canvas@2
// then run: npm run generate:og
// This script will no-op (exit 0) if canvas is not installed.
...
try {
  ...
  console.warn('[generate:og] canvas module not installed. Skipping OG image generation.');
  process.exit(0);
}
