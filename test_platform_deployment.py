...
def main():
    ...
    for path, content_check in test_cases:
        ...
        if result["success"]:
            ...
            reason = (
                "Content check failed" if not result["content_ok"] else "Status check failed"
            )
    ...
    percent = success_count / total_count * 100 if total_count else 0
    print(f"\nSuccess Rate: {success_count}/{total_count} ({percent:.1f}%)")
